...
abstract class BaseMarkupTestFixture<TBindable> : BaseMarkupTestFixture where TBindable : BindableObject, new()
{
	...
	protected void TestPropertiesSet<TPropertyValue>(
		...
		params List<(BindableProperty property, TPropertyValue beforeValue, TPropertyValue expectedValue)> propertyChanges)
		=> TestPropertiesSet(Bindable, modify, propertyChanges);

	protected void TestPropertiesSet(
		Action<TBindable> modify,
		params List<(BindableProperty property, object? beforeValue, object? expectedValue)> propertyChanges)
		=> TestPropertiesSet(Bindable, modify, propertyChanges);

	protected void TestPropertiesSet(
		Action<TBindable> modify,
		params List<(BindableProperty property, object? expectedValue)> propertyChanges)
		=> TestPropertiesSet(Bindable, modify, propertyChanges);
}
...
abstract class BaseMarkupTestFixture : BaseTestFixture
{
	protected static void TestPropertiesSet<TBindable, TPropertyValue>(
		TBindable bindable,
		Action<TBindable> modify,
		params List<(BindableProperty property, TPropertyValue beforeValue, TPropertyValue expectedValue)> propertyChanges) where TBindable : BindableObject
	{
	...
	protected static void TestPropertiesSet<TBindable, TPropertyValue>(
		TBindable bindable,
		Action<TBindable> modify,
		params List<(BindableProperty property, TPropertyValue expectedValue)> propertyChanges) where TBindable : BindableObject
	{
